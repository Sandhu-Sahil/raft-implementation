...
import (
	"log"
	"sandhu-sahil/raft-implementation/raft"
	"sync"
	"testing"
	"time"
)
...
func (h *Harness) CheckCommitted(cmd int) (nc int, index int) {
	...
	// Check consistency of commits from the start and to the command we're asked about. This loop will return once a command=cmd is found.
